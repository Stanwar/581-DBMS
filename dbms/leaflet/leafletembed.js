  ...
  //  console.log("shortest_block_id = "+shortest_block_id);
    ...
    //var str1  = 612312;
     ...
     var js;
        ...
        //request for the data from the server
        //xmlhttp.open("GET","test.php?l1="+l1+"&l2="+l2,true);
        xmlhttp.open("GET","dbupdate.php?q="+shortest_block_id+"&ts="+shortest_timestamp+"&c="+congestion_level,true);
        xmlhttp.send();
    
  document.getElementById("disp").innerHTML = "Database updated successfully";

};

//logic for the maintanance of the block information whenever some changes to the db are made.
...
//reset and maintance logic from here
var reset = function()
{
   ...
   if(js_reset == false)                                //if database not reverted back to the normal state
   {
   document.getElementById("disp").innerHTML = "Database failed to revert back to initial state";
   return; 
   }

   changedblocks.length = 0;
   document.getElementById("disp").innerHTML = "Database reverted back successfully";
};

//database maintanance code here. It is to revert back the database (availability for the suitable ongestion level) after the algo is finished running
var reset_db = function(bid, ts)
{
        ...
        xmlhttp.onreadystatechange = function() 
        {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
              ...
              //get the json response from the php file
                js_reset = JSON.parse(xmlhttp.responseText);
      //          console.log(js_reset);
        ...
        //console.log(congestion_level);
        xmlhttp.open("GET","db_reset.php?q="+bid+"&ts="+ts+"&c="+congestion_level,true);    //call to the database update file.
        xmlhttp.send();
 };